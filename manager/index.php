	...
	<div class="login">
		<form class="login-container validate-form" name="login-form" action="index.php" method="POST" onsubmit="return validateLoginForm()">
			...
			<?php
				...
				{
					$manager_username = test_input($_POST['username']);
					...
					$stmt->execute(array($manager_username,$hashedPass,$user_role));
					$row = $stmt->fetch();
					$count = $stmt->rowCount();
					...
					if($count > 0)
					{
						...
						$_SESSION['username_restaurant_qRewacvAqzA'] = $manager_username;
			...
			<p>
				<button type="submit" name="manager_login" >Sign In</button>
			</p>
			<form method="post">
